...
class H264Encoder {
    ...
    // Callback queue
    private let callbackQueue = DispatchQueue(label: "h264.encoder.callback")
    ...
    func start() -> Bool {
        ...
        guard status == noErr, let session = compressionSession else {
            logger.error("Failed to create H.264 compression session: \(status)")
            return false
        }
    ...
    private func configureEncoderForLowLatency(_ session: VTCompressionSession) {
        ...
        logger.info("H.264 encoder configured for low latency")
    }
...
private let compressionOutputCallback: VTCompressionOutputCallback = { (outputCallbackRefCon, sourceFrameRefCon, status, infoFlags, sampleBuffer) in
    ...
    let encoderInstance = Unmanaged<H264Encoder>.fromOpaque(encoder).takeUnretainedValue()
    ...
    encoderInstance.delegate?.h264Encoder(encoderInstance, didOutputSampleBuffer: sampleBuffer, isKeyframe: isKeyframe)
}